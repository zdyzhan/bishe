...
TurboIsoBoosted::TurboIsoBoosted(AdjacenceListsGRAPH * pHyperGraph, std::vector<int> * pNumberOfEmbeddings, ComboLinkedLists * pComboLinkedLists) {
...
void TurboIsoBoosted::setParameters(AdjacenceListsGRAPH * pQueryGraph, int * pPartialEmbedding, bool pNeedToSaveCache) {

	//cout << "TurboIsoBoosted Preparing Data -\/-" << endl; //@debug
...
void TurboIsoBoosted::clean_before() {

	necTree.swap(std::vector<NECNode>());

	startingQueryVertexCandidates.swap(std::queue<int>());
	scRootCandidates.swap(std::map<int, vector<int>* >());

}

void TurboIsoBoosted::clean_after() {
}
...
TurboIsoBoosted::NECNode * TurboIsoBoosted::nextQueryVertex()
{
	for (int i = 0; i < queryMatchingSuquence.size(); i++) {
		if (partialEmbedding[necTree[queryMatchingSuquence[i]].vertexId] == -1) {
...
bool TurboIsoBoosted::isJoinable(AdjacenceListsGRAPH::Vertex * u, AdjacenceListsGRAPH::Vertex * v) {
	...
	for (AdjacenceListsGRAPH::link t = u->adj; t != 0; t = t->next) {
		...
		if (partialEmbedding[t->v] != -1) {
			...
			else if (!hyperGraph->edge(v->id, partialEmbedding[t->v])) {
				return false;
			}
		}
		/*
...
void TurboIsoBoosted::dynamicLoadCandidates(int dataVertexId) {
	...
	*/
	AdjacenceListsGRAPH::Vertex* mappedVertex = hyperGraph->getVertexAddressByVertexId(dataVertexId);
	...
	*/
	for (std::vector<int>::iterator qdChildIterator = mappedVertex->qdContainedVertexList.begin(); qdChildIterator != mappedVertex->qdContainedVertexList.end(); qdChildIterator++) {
		AdjacenceListsGRAPH::Vertex* qdChildVertex = hyperGraph->getVertexAddressByVertexId(*qdChildIterator);
...
void TurboIsoBoosted::generateEquivalentEmbedding() {
	...
	*/
	int startQueryVertexMappingId = partialEmbedding[startQueryVertex];
	...
	qdChildIterator++) {
		partialEmbedding[startQueryVertex] = *qdChildIterator;
	...
	// recover
	if (partialEmbedding[startQueryVertex] != startQueryVertexMappingId) {
		partialEmbedding[startQueryVertex] = startQueryVertexMappingId;
	}
}
