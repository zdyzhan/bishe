...
void AdjacenceListsGRAPH::buildDFSTraversalOrder(){
	...
	while(!S.empty()){
		...
		if(!flags[id]){
			flags[id] = true;
			DFSTraversalOrder.push_back(&vertexList[id]);
...
void AdjacenceListsGRAPH::buildComboGraph(int maximumWidth){
	...
	for (std::map<int, std::vector<int>>::reverse_iterator levelInverseIterator = levelVertexInverted.rbegin(); levelInverseIterator != levelVertexInverted.rend(); levelInverseIterator++) {
		for (int i = 0; i < levelInverseIterator->second.size(); i++) {
			...
			for (int j = i+1; j < levelInverseIterator->second.size();j++) {
	...
	for(int i=0; i<comboGraph.size(); i++){
		for(int j=i+1; j<comboGraph.size(); j++){
			for(int k = 0; k< comboGraph[i].queryVertices.size(); k++){
				for(int l=0; l< comboGraph[j].queryVertices.size();l++){
...
void AdjacenceListsGRAPH::cleanComboInfo() {
	...
	comboGraph.swap(std::vector<ComboNODE>());
	delete comboGraphEdgeMatrix;
	DFSComboVertexOrder.swap(std::vector<int>());
...
void AdjacenceListsGRAPH::clear(){
	...
	vertexList.swap(std::vector<Vertex>());
	edgeList.swap(std::vector<Edge>());
	DFSTraversalOrder.swap(std::vector<Vertex *>());

	labelSet.swap(std::set<int>());
	labelVertexList.swap(std::map<int, std::vector<int>>());
	vertexLabelsEdgeList.swap(std::map<std::pair<int, int>, std::vector<std::pair<int, int>>>());

	TLSequenceMap.swap(std::map<TLSequence, std::vector<std::vector<int>>>());
	totalTLSNumber = 0;

	comboGraph.swap(std::vector<ComboNODE>());
	DFSComboVertexOrder.swap(std::vector<int>());

	labelContainmentRootList.swap(std::map<int, std::vector<int>>());
}
...
std::vector<AdjacenceListsGRAPH::Vertex *> * AdjacenceListsGRAPH::getDFSTraversalOrder(){
